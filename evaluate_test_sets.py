...
def evaluate_segmentation_test_set():
    print("=" * 70)
    print("  PHASE 4A: UNET SEGMENTATION EVALUATION ON UNSEEN TEST SET")
    print("=" * 70)
    ...
    print(f"Loaded {len(filenames)} unseen test image-mask pairs from '{test_img_dir}'.")
    ...
    with torch.no_grad():
        for fname in filenames:
            ...
            img = Image.open(ipath).convert('L').resize((256, 256), Image.Resampling.BILINEAR)
            mask = Image.open(mpath).convert('L').resize((256, 256), Image.Resampling.NEAREST)
    ...
    avg_dice = total_dice / len(filenames)
    avg_iou = total_iou / len(filenames)
    
    print(f"  Test Dice Score (Mean) : {avg_dice:.4f} ({avg_dice*100:.2f}%)")
    print(f"  Test IoU Score (Mean)  : {avg_iou:.4f} ({avg_iou*100:.2f}%)")
    return avg_dice, avg_iou

def evaluate_classification_test_set():
    print("\n" + "=" * 70)
    print("  PHASE 4B: CNN CLASSIFIER EVALUATION ON UNSEEN TEST SET")
    print("=" * 70)
        ...
        for c_idx, cname in enumerate(CLASSES):
            ...
            for fname in sorted(os.listdir(cdir)):
                if fname.endswith('.png'):
                    ...
                    img = Image.open(fpath).convert('L').resize((256, 256), Image.Resampling.BILINEAR)
                    img_arr = np.array(img, dtype=np.float32)
                    min_v, max_v = img_arr.min(), img_arr.max()
                    norm_img = (img_arr - min_v) / (max_v - min_v + 1e-8) if max_v > min_v else img_arr / 255.0
    ...
    print(f"  Test Images Evaluated  : {len(y_true)} (15 per class)")
